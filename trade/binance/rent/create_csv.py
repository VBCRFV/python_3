...
if __name__ == '__main__':
    ...
    ####################################################
    ### Создание переменных ###
    const = {'BTC': {'b': 'Antminer S9', 'c': 855, 'l': 1.335,'n': 2.5,'s': 0.24},
             'ETH': {'b': 'GPU', 'c': 150, 'l': 1.4,'n': 2.5,'s': 0.24},
             'LTC': {'b': 'Antminer L3', 'c': 24, 'l': 1,'n': 2.5,'s': 0.24}}
    txt_a = ""              # Дата
    ...
    ### Заполнение строк ###
    #txt = ""
    txt = str("Дата;Тип оборудования;Количество оборудования;Актив;Количество вх;Штамп времени продажи;Количество проданного;Курс продажи;Получено USDT;Комиссия от продажи USDT;Выручка от продажи;Коэфф Потребления;Общее кВт за сутки;Ставка тариф в руб;Курс ЦБ;Ставка тариф в USD;Расходы за э/э;Валовая прибыль;Комиссия за вывод;Прибыль;Выведено USDT;Уровень розетки;Остаток USDT;Остаток актива\n") #
    for el0 in db:
        txt_a = el0
        print(el0)
        for el1 in db[el0]:
            ...
            if el1 != 'USDT':
                ...
                text_write('csv/'+db_name.split('.')[0]+'_export_'+str(txt_d)+'.csv', csv_line, mode='a', encoding='utf8')
            txt = txt + csv_line
            csv_line = ""
    print(txt)
    #txt = txt.replace(".", ",").replace("1970-01-01T08:00:01", "0000-00-00 00:00:00")
    text_write('csv/export.csv', txt, mode='w', encoding='utf8')
